# 1. GASTOS DE APERTURA (Inversión Inicial)

Costos únicos necesarios para establecer la cafetería antes de comenzar operaciones.

## A) Equipamiento de Café

| Item | Cantidad | Costo Unitario | Costo Total |
|---|---|---|---|
| Máquina de espresso | 1 | $8,160.00 | $8,160.00 |
| Molino de café | 1 | $12,273.00 | $12,273.00 |
| Grinder de granos | 1 | | |
| Vapor de leche | Incluido | | |
| Máquina de hielo | 1 | | |
| Refrigerador | 1 | | |
| Congelador | 1 | | |
| **SUBTOTAL EQUIPAMIENTO** | | | |

---

## B) Muebles y Decoración

| Item | Cantidad | Costo Unitario | Costo Total |
|---|---|---|---|
| Mostrador/Barra | 1 | | |
| Mesas | | | |
| Sillas | | | |
| Estantes | | | |
| Decoración interior | | | |
| Rotulería/Señalética | 1 | | |
| **SUBTOTAL MUEBLES Y DECORACIÓN** | | | |

---

## C) Remodelación y Construcción

| Item | Cantidad | Costo Unitario | Costo Total |
|---|---|---|---|
| Pintura y acabados | 1 | | |
| Piso | m² | | |
| Instalación eléctrica | 1 | | |
| Instalación gas | 1 | | |
| Instalación hidráulica | 1 | | |
| Horno/Estufa | 1 | | |
| **SUBTOTAL REMODELACIÓN** | | | |

---

## D) Zona de Preparación

| Item | Cantidad | Costo Unitario | Costo Total |
|---|---|---|---|
| Campana extractora | 1 | | |
| Lavaplatos comercial | 1 | | |
| Lavamanos | 1 | | |
| Estantes de acero inoxidable | | | |
| Utensilios de cocina | 1 | | |
| **SUBTOTAL ZONA PREPARACIÓN** | | | |

---

## E) Sistema de Punto de Venta (POS)

| Item | Cantidad | Costo Unitario | Costo Total |
|---|---|---|---|
| Registradora/Sistema POS | 1 | | |
| Lector de tarjetas | 1 | | |
| Computadora | 1 | | |
| Impresora de recibos | 1 | | |
| **SUBTOTAL POS** | | | |

---

## F) Trámites, Permisos y Legales

| Item | Cantidad | Costo Unitario | Costo Total |
|---|---|---|---|
| Licencia de funcionamiento | 1 | | |
| Permiso sanitario (COEPRIS) | 1 | | |
| Registro mercantil | 1 | | |
| Asesoría legal | 1 | | |
| RFC y trámites fiscales | 1 | | |
| Seguro de responsabilidad | 1 | | |
| **SUBTOTAL TRÁMITES Y LEGALES** | | | |

---

## G) Seguridad y Vigilancia

| Item | Cantidad | Costo Unitario | Costo Total |
|---|---|---|---|
| Sistema de cámaras de seguridad | 1 | $4,000 | $4,000 |
| Instalación personal | 1 | - | - |
| **SUBTOTAL SEGURIDAD Y VIGILANCIA** | | | **$4,000** |

---

## H) Inventario Inicial

| Item | Cantidad | Costo Unitario | Costo Total |
|---|---|---|---|
| Café y especias | 1 | | |
| Leche y productos frescos | 1 | | |
| Vasos, servilletas, empaques | 1 | | |
| Otros consumibles | 1 | | |
| **SUBTOTAL INVENTARIO INICIAL** | | | |

---

## RESUMEN TOTAL DE GASTOS DE APERTURA

| Categoría | Costo Total |
|---|---|
| Equipamiento de Café | |
| Muebles y Decoración | |
| Remodelación y Construcción | |
| Zona de Preparación | |
| Sistema POS | |
| Trámites, Permisos y Legales | |
| Seguridad y Vigilancia | $4,000 |
| Inventario Inicial | |
| **TOTAL INVERSIÓN INICIAL** | |

---

## Notas

- Se requiere información del mercado local de Guadalajara para completar esta sección
- Algunos items pueden variar según la ubicación y tamaño del local
